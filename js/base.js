...
availableDatasets = {
    "MDG_0000000001":{
       "Title":"Infant mortality rate (probability of dying between birth and age 1 per 1000 live births)",
       "Unit":"Deaths",
       "Size":7891384,
       "Reversed":true
    },
    "MDG_0000000003":{
       "Title":"Adolescent birth rate (per 1000 women aged 15-19 years)",
       "Unit":"per 1000 women",
       "Size":528683,
       "Reversed":true
    },
    "MDG_0000000007":{
       "Title":"Under-five mortality rate (probability of dying by age 5 per 1000 live births)",
...
document.getElementById("loading-text").innerHTML = "Downloading Information from WHO... (This may take a few moments)"
bar.set(0)
...
// search field two setup
var searchFieldTwo = $('#searchFieldTwo');
...
// new stuff
var data = {};
function convertData(whoType, whoData){
    ...
    whoData.fact.forEach(function(x) {
        ...
        if(!isNaN(year) ){
            ...
            newCountryData = {
                "countryCode": country_convert[country],
                "value": value
            }
            data.data[year][whoType].push(newCountryData)
        }
...
function prepareDataField(whoType, whoName, whoUnit, whoData){
    document.getElementById("loading-text").innerHTML = "Preparing Dataset..."
    largestYear = 0
    data["data"] = {};
    whoData.fact.forEach(function(x) {
        if(!isNaN(x.dim.YEAR) ){
            if(!(x.dim.YEAR in data.data)){
                data.data[x.dim.YEAR] = {};
                data.data[x.dim.YEAR][whoType] = [];
                }
            if(parseInt(x.dim.YEAR) > largestYear){
                largestYear = parseInt(x.dim.YEAR);
            }
        }
    })
    // Hack together indicator data
    data["defaultYear"] = largestYear.toString()
    data["defaultIndicator1"] = whoType
    data["defaultIndicator2"] = whoType
    data["defaultIndicator3"] = whoType
    data["indicators"] = {
        [whoType]: {
            "name": whoName,
            "unit": whoUnit
...
if(urlParams.get("code") == null){
    ...
    whoCode = urlParams.get("code")
}

if(urlParams.get("year") != null){
    defYear = urlParams.get("year")
}
whoTitle = availableDatasets[whoCode].Title
whoUnit = availableDatasets[whoCode].Unit

document.getElementById("loading-currently-viewing").innerHTML = "Currently Loading: " + whoTitle
...
for (var datasetname in availableDatasets) {
    // add country names and capitals to be used on auto complete search field
    autoCompleteLookupTwo.push({
        value: availableDatasets[datasetname].Title,
        data: datasetname
    });
}
searchFieldTwo.on('focus', function() {
    var $this = $(this)
    ...
    lookup: autoCompleteLookupTwo,
    onSelect: function (suggestion) {
        selectTwo(suggestion.data);
        searchFieldTwo.val('');
        searchFieldTwo.blur();
    }
});
...
function getWhoData(whoType, attempt){
    ...
    xmlHttp.onload = function() {
        if (xmlHttp.status != 200) { // analyze HTTP status of the response
            if(attempt <= 3){
                getWhoData(whoType, attempt+1)
            }
            else{
                console.log(`Error ${xmlHttp.status}: ${xmlHttp.statusText}`); // e.g. 404: Not Found
                document.getElementById("loading-text").innerHTML = "This Dataset Is Currently Not Available. Try Again Later."
            }
        } else { // show the result
            whoData = JSON.parse(xmlHttp.responseText);
            //return xmlHttp.responseText;
            console.log("Parsing Data")
            netCallbackThing(whoData)
        }
    };
    xmlHttp.onprogress = function (event) {
        totalSize = availableDatasets[whoType].Size
        console.log("LOADED - " + event.loaded + " " + totalSize + " " + Math.floor((event.loaded/totalSize)/2));
        document.getElementById("loading-text").innerHTML = "Downloading Information from WHO... (" + event.loaded + "/" + totalSize + ")"
        a = Math.floor((event.loaded/totalSize)*100)
        bar.set(a)
      };
...
function updateCopyLinkArea(){
    document.getElementById("linkArea").value = generateUrl();
}



/*
function getWhoData(whoType){
    document.getElementById("loading-text").innerHTML = "Downloading Information from WHO.... (This may take a few moments)"
    bar.set(0)   
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", "https://cors-anywhere.herokuapp.com/apps.who.int/gho/athena/api/GHO/" + whoType + "?format=json&profile=simple" );
    ...
    xmlHttp.onload = function() {
        if (xmlHttp.status != 200) { // analyze HTTP status of the response
            console.log(`Error ${xmlHttp.status}: ${xmlHttp.statusText}`); // e.g. 404: Not Found
            document.getElementById("loading-text").innerHTML = "(2) Downloading Information from WHO.... (This may take a few moments)"
            bar.set(0)   
            var xmlHttp = new XMLHttpRequest();
            xmlHttp.open("GET", "https://cors-anywhere.herokuapp.com/apps.who.int/gho/athena/api/GHO/" + whoType + "?format=json&profile=simple" );
            //xmlHttp.open("GET", "./data/" + whoType + ".json", false );
            ...
            xmlHttp.onload = function() {
                if (xmlHttp.status != 200) { // analyze HTTP status of the response
                    ...
                    xmlHttp.onprogress = function (event) {
                        ...
                        console.log("LOADED - " + event.loaded + " " + totalSize + " " + Math.floor((event.loaded/totalSize)/2));
                        document.getElementById("loading-text").innerHTML = "Downloading Information from WHO... (" + event.loaded + "/" + totalSize + ")"
                        a = Math.floor(((event.loaded/totalSize)*100)/2)
                        bar.set(a)
                    };
                } else { // show the result
                    whoData = JSON.parse(xmlHttp.responseText);
...
readJsonFile('data/gray_codes.json', function(gray_codes) {
    ...
    readJsonFile('data/countries.json', function(countries) {
        ...
        getWhoData(whoCode, 1)
    });
});


function netCallbackThing(whoData){
    console.log("PREPARING DATA FIELDS")
        prepareDataField(whoCode, whoTitle, whoUnit, whoData)
        console.log("CONVERTING DATA")
        convertData(whoCode, whoData);

        document.getElementById("stat-text").innerHTML = whoTitle

        document.getElementById("loading-text").innerHTML = "Rendering Globe..."
        bar.set(95)

        updateCopyLinkArea()

        document.getElementById("linkAreaText").onclick = function() {
            console.log("Copying!")
            document.getElementById("yoyoyo-cool-thing").innerHTML = "Copied!"
        ...
        if(defYear != undefined){
            selectedYear = defYear;
        }
        else{
